...
class AnchorHeadTemplate(nn.Module):
    ...
    def get_hm_loss(self):
        hm_preds = self.forward_ret_dict['pred_hm'].view(-1, 1)
        hm_labels = self.forward_ret_dict['gt_hm'].view(-1, 1)
      
        #   smooth-L1损失
        hm_loss_N = F.smooth_l1_loss(hm_preds, hm_labels, size_average=True)
        Gamma = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['hm_weight']
        # loss乘以热图权重 --> cls_weight=5.0
        hm_loss = Gamma * hm_loss_N/2
        print(f"hm_loss:{hm_loss}= Gamma*hm_loss_N/2, Gamma={Gamma}")
        tb_dict_hm = {
            'hm_cls': hm_loss.item()
        }
        return hm_loss, tb_dict_hm
...
# 也要减去45度再进行之后测nms等操作。
    ...
    def get_loss(self):
        ...
        # 计算热图损失 charles
        hm_loss, tb_dict_hm = self.get_hm_loss()
        # print("hm_loss add...hm_loss: ",hm_loss)
        # print("hm_loss add...hm_loss*10: ",hm_loss*10.0)
        tb_dict.update(tb_dict_hm)

        # rpn_loss = cls_loss + box_loss + hm_loss
        rpn_loss = cls_loss + box_loss + hm_loss
