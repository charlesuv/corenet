...
class HeightCompression_pointnet(nn.Module):
    def __init__(self, model_cfg, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        # 高度的特征数
        self.input_channels = self.model_cfg.get('INPUT_CHANNEL', 5) # tjradar为 8
        self.num_bev_features = self.model_cfg.NUM_BEV_FEATURES

        self.point_fc = nn.Linear(96, 64, bias=False)
        # 一层mlp
        self.point_cls = nn.Linear(64+128, 1, bias=False)
        # self.point_cls_mlp1 = nn.Linear(64+128+self.input_channels, 1, bias=False) # pointnet2生成的特征+pointwise_1+ resnet生成的特征-过1层mlp
        # self.point_cls_mlp1 = nn.Linear(128+self.input_channels, 1, bias=False) # pointnet2生成的特征+ resnet生成的特征-过1层mlp
        # # 三层mlp
        # self.point_cls1 = nn.Linear(128+64+self.input_channels, 64, bias=False)
        ...
        channel_in = self.input_channels - 3
        self.num_points_each_layer = []
        skip_channel_list = [self.input_channels - 3]
    ...
    def forward(self, batch_dict):
        ...
        if self.training and (int(batch_dict["mode"][0])==1):
            ...
            for idx in range(row_counts1.shape[0]):
                ...
                # # 分类和回归
                res_feature = voxel_features[start_idx : start_idx+row_counts1[idx], :].view(1, -1, self.input_channels).contiguous()
                
                # pointnet2生成的特征+pointwise_1-过1层mlp
                point_cls_1 = self.point_cls(torch.cat((pointwise_1, point_features), dim=-1))#torch.Size([1, N1, 1])
                print("pointnet2特征 + pointwise_1 -过1层mlp")

                # # pointnet2生成的特征+pointwise_1+ resnet生成的特征-过1层mlp
                # point_cls_1 = self.point_cls_mlp1(torch.cat((pointwise_1, point_features, res_feature), dim=-1))#torch.Size([1, N1, 1])
                # print("pointnet2特征 + pointwise_1 + resnet特征-过1层mlp")

                # # pointnet2生成的特征+ resnet生成的特征-过1层mlp
                # point_cls_1 = self.point_cls_mlp1(torch.cat((point_features, res_feature), dim=-1))#torch.Size([1, N1, 1])
                # print("pointnet2特征 + resnet特征-过1层mlp")

                # # pointnet2生成的特征+pointwise_1+ resnet生成的特征-过三层mlp
                # point_cls_1 = self.point_cls1(torch.cat((point_features, pointwise_1, res_feature), dim=-1))#torch.Size([1, 128+64+self.input_channels, 1])
                # point_cls_1 = self.point_cls2(point_cls_1)#torch.Size([1, 128+self.input_channels, 1])
                # point_cls_1 = self.point_cls3(point_cls_1)#torch.Size([1, 128+self.input_channels, 1])
                # print("pointnet2特征 + pointwise_1 + resnet特征-过三层mlp")
                # 分类和回归-消融实验: 无pointnet2
                # zero_vector = torch.zeros_like(point_features)
                # point_cls_1 = self.point_cls(torch.cat((pointwise_1, zero_vector), dim=-1))#torch.Size([1, N1, 1])
                ## point_cls_1 = self.point_cls(pointwise_1) #torch.Size([1, N1, 1]) 之前只保存了cnn输出的pointwise_1特征
        ...
        elif self.training and (int(batch_dict["mode"][0])==0):
