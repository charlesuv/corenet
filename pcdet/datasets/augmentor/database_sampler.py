...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene_del_empty(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        # charles delete
        indices_of_empty_lists = [i for i, x in enumerate(obj_points_list) if x.shape[0] == 0]
        sampled_gt_boxes = np.delete(sampled_gt_boxes, indices_of_empty_lists, axis=0)
        for index in sorted(indices_of_empty_lists, reverse=True):
            del obj_points_list[index]
            del total_valid_sampled_dict[index]
        ...
        # 将数据增强的点加入到match_points中
        data_dict["match_points"] = np.concatenate([data_dict["match_points"], obj_points], axis=0)
        if data_dict['debug_mode'] == 1:
            import os
            current_working_directory = os.getcwd()
            parent_dir = os.path.dirname(current_working_directory)
            np.save(parent_dir + '/output_debug/sample_match_points.npy', data_dict["match_points"])
            np.save(parent_dir + '/output_debug/sample_points.npy', points)
            np.save(parent_dir + '/output_debug/sample_gt_boxes.npy',gt_boxes)
            np.save(parent_dir + '/output_debug/sample_gt_names.npy',gt_names)
        # # charles 只保留car类别信息
        # indices_car = [index for index, value in enumerate(gt_names) if value == 'Car']
        # data_dict['gt_boxes'] = data_dict['gt_boxes'][indices_car]
        # data_dict['gt_names'] = data_dict['gt_names'][indices_car]
     ...
     # charles ############################################################
    def generate_match_points(self, points, points_lidar):
        from scipy.spatial import KDTree
        # 将array格式的点云转换为open3d的点云格式,若直接用open3d打开的点云数据，则不用转换
        pcd_arbe = points  # [N1, 4]
        pcd_lida = points_lidar  # [N2, 4]
        # 构建KD树
        kdtree_target = KDTree(pcd_lida[..., :3])
        # 查询最近邻点及距离
        distances, indices = kdtree_target.query(pcd_arbe[..., :3], k=1)
        limit_range = self.sampler_cfg.KDTREE_distance
        mask_all = np.where(distances < limit_range, True, False)
        # print(f"mask_all.shape: {mask_all.shape}")
        pcd_arbe_close2lidar_array = pcd_arbe[mask_all]

        return pcd_arbe_close2lidar_array
########################################################################################
    ...
    def __call__(self, data_dict):
        ...
        # charles #######################################################################
        match_points = self.generate_match_points(data_dict['points'], data_dict['points_lidar']) # charles
        # 当lidar点云为空，将match_points赋予radar点云
        if match_points.shape[0] == 0:
            match_points = data_dict['points']
        data_dict['match_points'] = match_points
        #################################################################################
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            # )
            data_dict = self.add_sampled_boxes_to_scene_del_empty(
                data_dict, sampled_gt_boxes, total_valid_sampled_dict, sampled_mv_height, sampled_gt_boxes2d
            )

        data_dict.pop('gt_boxes_mask')

        # 耦合match_points, points 删除points_lidar
        # if (data_dict["mode"] == 1):
        data_dict["arbe_num"] = data_dict["points"].shape[0]    # 15285
        data_dict["match_num"] = data_dict["match_points"].shape[0] # 12601
        data_dict["points"] = np.concatenate((data_dict["points"], data_dict["match_points"]), axis=0)
        # if data_dict['debug_mode'] == 1:
        #     data_dict["points"] = np.concatenate((data_dict["points"], data_dict["points_lidar"][:,:5]), axis=0)
        del data_dict["points_lidar"]
