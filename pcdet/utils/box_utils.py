...
def boxes3d_kitti_camera_to_lidar(boxes3d_camera, calib):
    ...
    xyz_lidar[:, 2] += h[:, 0] / 2
    # 因为我们的数据激光坐标系以y轴为前方 而kitti坐标系以x轴为前方 所以这里不用加np.pi / 2
    return np.concatenate([xyz_lidar, l, w, h, -(r + np.pi / 2)], axis=-1)

def boxes3d_dualradar_camera_to_lidar(boxes3d_camera, calib):
    """
    Args:
        boxes3d_camera: (N, 7) [x, y, z, l, h, w, r] in rect camera coords
        calib:
    ...
    """
    boxes3d_camera_copy = copy.deepcopy(boxes3d_camera)
    xyz_camera = boxes3d_camera_copy[:, 0:3]
    l, h, w, r = boxes3d_camera_copy[:, 3:4], boxes3d_camera_copy[:, 4:5], boxes3d_camera_copy[:, 5:6], boxes3d_camera_copy[:, 6:7]
    xyz_lidar = calib.rect_to_lidar(xyz_camera)
    xyz_lidar[:, 2] += h[:, 0] / 2
...
def boxes3d_lidar_to_dualradar_camera(boxes3d_lidar, calib):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
    ...
    boxes3d_lidar_copy = copy.deepcopy(boxes3d_lidar)
    xyz_lidar = boxes3d_lidar_copy[:, 0:3]
    l, w, h, r = boxes3d_lidar_copy[:, 3:4], boxes3d_lidar_copy[:, 4:5], boxes3d_lidar_copy[:, 5:6], boxes3d_lidar_copy[:, 6:7]
